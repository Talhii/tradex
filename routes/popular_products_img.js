...
//    });
    ...
    var pid = req.body.id;
    var pid = 1;
    pool.query(`INSERT INTO popular_images (Image_Name,Product_Id) VALUES ("${image_name}", "${pid}")`, function(err, row, fields) {
        if(err) {
...
router.get('/getimage',ensureToken, function(request, response) {
  pool.query("select * from popular_images ", function(err, result) {
