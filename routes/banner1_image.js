...
router.post('/', ensureToken ,upload.single('image'), function (req, res, next) {
    ...
    // var image_name = req.file.image;
   // var filepath = req.files.diskStorage;
   // console.log("disk storge " + filepath)
   ...
   console.log("photo="+image_name);
      ...
      fs.writeFileSync('Images/'+image_name, image);
       // delete file named 
       fs.unlink('Images/' + image_name, function (err) {
        if (err) throw err;
    ...
    pool.query("INSERT INTO banner_images value (?,?,?)", [ id, image_name ,user_id], function(err, row, fields) {
        if(err) {
...
router.get('/getimage',ensureToken ,function(request, response) {
  pool.query("select * from banner_images ", function(err, result) {
...
// fetch the request data
 ...
 // define the validation schema
  ...
  pool.query(`select * from banner_photos WHERE id=${id} `, function(err, result) {
    //  response.writeHead(200, {
     //     'Content-Type': 'image/jpeg'
     // });
      //console.log("HELLO");
