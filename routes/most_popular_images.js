...
//    });
    ...
    var pid = req.body.id;
    pool.query(`INSERT INTO mpp_images (Image_Name,Product_Id) VALUES ("${image_name}", "${id}")`, function(err, row, fields) {
        if(err) {
...
router.get('/getimage',ensureToken, function(request, response) {
  pool.query("select * from mpp_images", function(err, result) {
